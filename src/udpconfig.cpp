...
#include "4_mecanum.h"  // ✅ Motor 4 position control + test
#endif
#ifdef Omni
#include "uart.h"
#include "led.h"
...
static unsigned long udpBackoffUntil = 0;
...
// Đang trong thời gian backoff - lwIP đang hết buffer
if (now < udpBackoffUntil) {
xSemaphoreGive(udpMutex);
return false;
}

// Throttling - bỏ qua nếu gửi quá nhanh
if (now - lastUDPSendTime < UDP_THROTTLE_MS) {
xSemaphoreGive(udpMutex);
return false;
}

// Gửi packet, kiểm tra cả beginPacket và endPacket
if (touch_udp.beginPacket(touch_server_address, TOUCH_SERVER_PORT)) {
touch_udp.write((uint8_t*)message, strlen(message));
if (touch_udp.endPacket()) {
udpFailCount = 0;
} else {
// endPacket thất bại (ENOMEM): tăng đếm lỗi
if (++udpFailCount >= 3) {
udpBackoffUntil = millis() + 2000;  // Dừng gửi 2 giây
udpFailCount = 0;
}
}
}
...
// SET_THRESH_B{0|1}:t0,t1,...,t15
if (data.startsWith("SET_THRESH_B")) {
    int boardId = buffer[12] - '0';
    Serial.printf("[THRESH] Nhan B%d tu %s:%d\n", boardId,
    ...
    if (boardId == 0 || boardId == 1) {
        ...
        if (p) {
            ...
            Serial.printf("[THRESH] B%d: parsed %d values (first=%d last=%d)\n", 
                boardId, parsedCount, firstVal, lastVal);
            Serial.printf("[THRESH] B%d verify: [0]=%d [1]=%d [15]=%d\n", boardId,
                muxThreshold[boardId][0], muxThreshold[boardId][1],
                muxThreshold[boardId][15]);
            
            // ✅ FIX: Force send IR_B0/IR_B1 immediately after threshold update
            // so frontend sees the LED status change with new thresholds
            delay(100);  // Give scanMux time to compute new digital values
            sendIRBoard(boardId);
            Serial.printf("[THRESH] B%d: Sent IR_BOARD message immediately\n", boardId);
        }
    }
}
...
if (fabsf(revs) < 0.1f) revs = 5.0f;
if (rpm <= 0) rpm = 60.0f;
motorStartTest4(revs, rpm);
} else if (rest.startsWith("REVS")) {
int r1 = rest.indexOf(',');
int r2 = (r1 >= 0) ? rest.indexOf(',', r1 + 1) : -1;
if (r1 < 0) {
Serial.println("[UDP_MOTOR] M4 REVS format: MOTOR:4,REVS,revs[,rpm]");
} else {
float revs = rest.substring(r1 + 1, (r2 > 0) ? r2 : rest.length()).toFloat();
float rpm  = (r2 >= 0) ? rest.substring(r2 + 1).toFloat() : 60.0f;
if (revs == 0) {
mecanumSetM4TargetRPM(0.0f);
Serial.println("[UDP_MOTOR] M4 STOP");
} else {
motorRunRevs4(revs, rpm);
Serial.printf("[UDP_MOTOR] M4: %.1f vong @ %.1f RPM\n", revs, rpm);
}
}
...
Serial.printf("[UDP_MOTOR] M4 lenh khong ro: %s\n", rest.c_str());
}
} else
#endif
{
...
if (!m) {
Serial.printf("[UDP_MOTOR] Motor %d khong ton tai (dung 1/2/3/4)\n", motorNum);
...
Serial.printf("[UDP_MOTOR] Lenh khong ro: %s\n", rest.c_str());
}
} // end else (motor 1/2/3 block)
}
}
...
void sendIRBoard(uint8_t board) {
    if (board >= 2) return;
    char buf[280];
    int pos = 0;

    // Header: IR_B0: hoặc IR_B1:
    pos += snprintf(buf + pos, sizeof(buf) - pos, "IR_B%d:", board);

    // Section 1: raw ADC (0-4095)
    for (int i = 0; i < MUX_CHANNELS; i++)
        pos += snprintf(buf + pos, sizeof(buf) - pos,
                        i ? ",%d" : "%d", muxData[board][i]);

    // Section 2: threshold per LED
    pos += snprintf(buf + pos, sizeof(buf) - pos, "|");
    for (int i = 0; i < MUX_CHANNELS; i++)
        pos += snprintf(buf + pos, sizeof(buf) - pos,
                        i ? ",%d" : "%d", muxThreshold[board][i]);

    // Section 3: digital state (16 ký tự 0/1 liên tiếp)
    pos += snprintf(buf + pos, sizeof(buf) - pos, "|");
    for (int i = 0; i < MUX_CHANNELS; i++)
        pos += snprintf(buf + pos, sizeof(buf) - pos, "%d", muxDigital[board][i]);

    // ✅ DEBUG: Log packet being sent
    static unsigned long lastLog = 0;
    if (millis() - lastLog > 1000) {
        lastLog = millis();
        Serial.printf("[UDP_SEND] B%d digital bits: ", board);
        for (int i = 0; i < MUX_CHANNELS; i++)
            Serial.printf("%d", muxDigital[board][i]);
        Serial.printf("\n[UDP_SEND] Packet: %s\n", buf);
    }

    sendUDPPacket(buf, UDP_PRIORITY_LOW);
}

void sendIRAllBoards() {
    static uint16_t lastSent[2][MUX_CHANNELS] = {};
    static uint8_t lastSentDigital[2][MUX_CHANNELS] = {};
    static const uint16_t DELTA = 80;  // ngưỡng thay đổi ADC để gửi
    static unsigned long lastLog = 0;

    for (uint8_t b = 0; b < 2; b++) {
        bool changed = false;
        
        // ✅ Check 1: Raw ADC value changed significantly
        for (uint8_t ch = 0; ch < MUX_CHANNELS; ch++) {
            if (abs((int)muxData[b][ch] - (int)lastSent[b][ch]) > DELTA) {
                changed = true;
                break;
            }
        }
        
        // ✅ Check 2: Digital state changed (CRITICAL FIX!)
        if (!changed) {
            for (uint8_t ch = 0; ch < MUX_CHANNELS; ch++) {
                if (muxDigital[b][ch] != lastSentDigital[b][ch]) {
                    changed = true;
                    if (millis() - lastLog > 500) {
                        Serial.printf("[SEND_IR] B%d CH%d digital: %d→%d\n", 
                            b, ch, lastSentDigital[b][ch], muxDigital[b][ch]);
                    }
                    break;
                }
            }
        ...
        if (changed) {
            sendIRBoard(b);
            memcpy(lastSent[b], muxData[b], MUX_CHANNELS * sizeof(uint16_t));
            memcpy(lastSentDigital[b], muxDigital[b], MUX_CHANNELS * sizeof(uint8_t));
            if (millis() - lastLog > 2000) {
                Serial.printf("[SEND_IR] B%d sent (raw/digital changed)\n", b);
                lastLog = millis();
            }
        }
    }
